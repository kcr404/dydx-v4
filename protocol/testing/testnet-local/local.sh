...
create_validators() {
	...
	for i in "${!MONIKERS[@]}"; do
		...
		for acct in "${TEST_ACCOUNTS[@]}"; do
			dydxprotocold add-genesis-account "$acct" 100000000000000000$USDC_DENOM,100000000000000000ibc/570913205C935953028BF658FA4D916C4977D179F938477EC4722513B82060F3,$TESTNET_VALIDATOR_NATIVE_TOKEN_BALANCE$NATIVE_TOKEN --home "$VAL_HOME_DIR"
		done
		for acct in "${FAUCET_ACCOUNTS[@]}"; do
			dydxprotocold add-genesis-account "$acct" 900000000000000000$USDC_DENOM,900000000000000000ibc/570913205C935953028BF658FA4D916C4977D179F938477EC4722513B82060F3,$TESTNET_VALIDATOR_NATIVE_TOKEN_BALANCE$NATIVE_TOKEN --home "$VAL_HOME_DIR"
		done
